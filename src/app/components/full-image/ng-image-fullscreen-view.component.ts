...
export class NgImageFullscreenViewComponent implements OnDestroy {
    ...
    speed: number = 0.5; // default speed in second
    title: string = '';
    ...
    @ViewChild('lightboxDiv',{static: true}) lightboxDiv;
    @ViewChild('lightboxImageDiv',{static:true}) lightboxImageDiv;
    ...
    set imageIndex(index: number) {
        console.log('set imageIndex',index)
    ...
    onDel(){
        // console.log(this.images[this.currentImageIndex].id)
        this.del.emit({images:this.images,index:this.currentImageIndex});
    }
